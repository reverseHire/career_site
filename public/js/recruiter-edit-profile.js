$(document).ready(function () {
    ...
    function setJobDescObjectToForm(i, jobDesc) {
        $('#exp-' + i).find('.company').attr('value', jobDesc.company)
        $('#exp-' + i).find('.job-title').attr('value', jobDesc.jobTitle)
        $('#exp-' + i).find('.description').html(jobDesc.description)
        $('#exp-' + i).find('.start-date').attr('value', getConvertedDate(jobDesc.startDate))
        var endDate = getConvertedDate(jobDesc.endDate)
        setWExpEndDateonForm(jobDesc, i, endDate);
    }

    function cloneJobDesc(id) {
        var jobDescClone = $('#work-exp-frame').clone()
        $(jobDescClone).attr('id', 'exp-' + id)
        $(jobDescClone).appendTo(".work-exp-container")
